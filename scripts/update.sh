...
version=$(curl --silent "https://api.github.com/repos/dalance/procs/releases/latest" | jq -er .tag_name)
revision=$(curl --silent "https://api.github.com/repos/dalance/procs/commits/${version}" | jq -er .sha)
version=${version#"v"}
